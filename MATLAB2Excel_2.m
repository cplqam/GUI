function MATLAB2Excel_2(identif)
...
for n = 1:r_c
    ...
    if isempty(identifications)
        new_identifier{n,2}{1,1} = 0;
        new_identifier{n,2}{1,2} = 0;
        new_identifier{n,2}{1,3} = 0;
        new_identifier{n,2}{1,4} = 0;
        new_identifier{n,2}{1,5} = 0;
        new_identifier{n,2}{1,6} = 0;
        new_identifier{n,2}{1,7} = 0;
    else 
...
new_identifier_fil = {};
for n = 1:size(new_identifier,1)
    % key = unique([new_identifier{n,2}{:,7}]);
    max_val = max([new_identifier{n,2}{:,4}]);
    idx = find([new_identifier{n,2}{:,4}] == max_val);
    new_identifier{n,2} = new_identifier{n,2}(idx,:);
    new_identifier_fil{n,1} = new_identifier{n,1};
    new_identifier_fil{n,2} = new_identifier{n,2};
    % for inch = 1:size(key,2)
    %     ik = find([new_identifier{n,2}{:,7}] == key(inch));
    %     ik_comps = [new_identifier{n,2}(ik,:)];
    %     [s,p] = max([ik_comps{:,4}]);
...
new_identifier_def = new_identifier_def(:,[1,2,5]);
